...
const Commands = {
  ...
  ai: function(data) {
    ...
    for (let i = 0; i < Number(data[5]); i++) servers[this.room].ai.push(new AI(Math.floor(Number(data[1]) / 100) * 100 + 10, Math.floor(Number(data[2]) / 100) * 100 + 10, Number(data[3]), Number(data[4]), ':'+data[6], servers[this.room]));
  },
