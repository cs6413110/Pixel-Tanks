const settings = {
  ...
  admins: ['Celestial', 'bradley', '3foenation', 'DarkMemeGod', 'LostKing'], 
  vips: ['tony34'],
  mutes: ['3foe'],
...
wss.on('connection', socket => {
  ...
  socket.on('message', data => {
    ...
    if (data.type === 'update') {
      ...
      servers[socket.room].logs.push({m: `[${socket.username}] ${data.msg.slice(0, 100)}`, c: '#ffffff'});
      log(`[${socket.username}] ${data.msg.slice(0, 100)}`);
