...
const Commands = {
  ...
  invis: [Object, 2, 3, function(data) {
    ...
    for (let i = 0; i < (data[2]*160); i++) setTimeout(() => {
      if (t) servers[this.room].d.push(A.template('Damage').init(Math.round(t.x+Math.floor(Math.random()*350)-250), Math.round(t.y+Math.floor(Math.random()*350)-250), 200, 200, 0, t.team, t.host)), Math.random()*(data[2]*1000));
    }
  }],
