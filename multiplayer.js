...
wss.on('connection', socket => {
  ...
  socket.on('message', data => {
    ...
    if (data.type === 'update') {
      ...
      gpt({model: 'text-babbage-001', prompt: 'You are a chat filterer for an online chat implemented in a game engine. Filter the following message accordingly of any profanity or bullying, but stay within reason. Your outputs should be in a simple single-line json format of {"msg":"<filtered message>"}. Do not have any commentary of any kind preceding the filtered message. Here is the message: '+data.msg, markdown: false}, (err, data) => {
        if (servers[socket.room]) servers[socket.room].logs.push({m: `[${socket.username}] ${JSON.parse(data.gpt).msg}`, c: '#ffffff'});
      });
