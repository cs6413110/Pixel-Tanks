class PixelTanks {
    ...
    Menus.menus = {
      ...
      inventory: {
        ...
        cdraw: function() {
          ...
          if (PixelTanks.userData.cosmetic_hat) GUI.drawImage(PixelTanks.images.cosmetics[PixelTanks.userData.cosmetic_hat], 448, 360, 88, 88, 1);
          if (PixelTanks.userData.cosmetic) GUI.drawImage(PixelTanks.images.cosmetics[PixelTanks.userData.cosmetic], 448, 460, 88, 88, 1);
          if (PixelTanks.userData.cosmetic_body) GUI.drawImage(PixelTanks.images.cosmetics[PixelTanks.userData.cosmetic_body], 448, 560, 88, 88, 1);
          ...
          if (this.classTab) {
            ...
            for (let i = this.cosmeticMenu*16; i < Math.min((this.cosmeticMenu+1)*16, PixelTanks.userData.cosmetics.length); i++) {
              ...
              GUI.drawText(PixelTanks.userData.cosmetics[i].split('#')[1], 598+(i%4)*108, 298+Math.floor((i%16)/4)*108, 30, '#FFE900', .5);
  ...
  static openCrate(type) {
    const price = type ? 5 : 1, name = type ? 'deathEffects' : 'cosmetics', rand = Math.floor(Math.random()*1001), crate = [{
      common: ['Spooked', 'Cute Eyes', 'America', 'Army', 'UnionJack', 'Top Hat', 'SKOTTISH', 'X', 'Red Hoodie', 'Devil Wings', 'Devil Horns', 'Exclaimation Point', 'Orange Hoodie', 'Gold Shield', 'Yellow Hoodie', 'Green Hoodie', 'Blue Hoodie', 'Purple Hoodie', 'Cancelled', 'Spirals', 'Speaker', 'Spikes', 'Candy Cane', 'Pumpkin Face', 'Mask', 'Purple-Pink Hoodie', 'Bunny Ears', 'Red Ghost', 'Blue Ghost', 'Pink Ghost', 'Orange Ghost'],
      uncommon: ['Anime Eyes', 'Angry Eyes', 'Hard Hat', 'Present', 'Dead', 'Peace', 'Question Mark', 'Small Scratch', 'Kill = Ban', 'Reindeer Hat', 'Pumpkin Hat', 'Cat Ears', 'Cake', 'Cat Hat', 'First Aid', 'silver', 'Fisher Hat'],
      rare: ['Stripes', 'scoped', 'brain', 'hands', 'Peashooter', 'Box', 'Straw Hat', 'Hax', 'Tools', 'Money Eyes', 'Dizzy', 'Checkmark', 'Sweat', 'Scared', 'Blue Tint', 'Purple Top Hat', 'Purple Grad Hat', 'Eyebrows', 'Helment', 'Rudolph', 'Candy Corn', 'Flag', 'Katana',  'Swords'],
      epic: ['Hazard', 'Locked', 'Elf', 'Triple Gun', 'Black', 'Evil Eyes', 'Gold', 'Rage', 'Onfire', 'Halo', 'Police', 'Deep Scratch', 'bluekatana', 'Back Button', 'Assassin', 'Astronaut', 'Christmas Lights', 'No Mercy', 'Error'],
      ...
      common: ['erase', 'explode', 'nuke', 'evan', 'banhammer'], //bruh why am i common :(
      uncommon: ['ghost', 'anvil', 'insta', 'knight', 'gameover'],
      rare: ['amogus', 'minecraft', 'magic', 'kermit', 'plant'],
      epic: ['wakawaka', 'battery', 'fix', 'mechagodzilla'],
      legendary: ['error', 'enderman', 'crate'],
