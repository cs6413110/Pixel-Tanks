class Client {
  ...
  drawShot(s) {
    if (s.type == 'bullet') {
      ...
      let l = Engine.hasPerk(PixelTanks.userData.perk, 7);
      if (l) return GUI.drawImage(PixelTanks.images.bullets.yoink, s.x-22.5, s.y-22.5, 45, 45, 1, 22.5, 22.5, 0, 0, s.r+90);
      ...
      l = Engine.getUsername(s.team) === 'DarkMemeGod';//too late u already did
      if (l && PixelTanks.user.username === 'DarkMemeGod') {
        GUI.drawImage(PixelTanks.images.bullets.rust, s.x-22.5, s.y-22.5, 45, 45, 1, 22.5, 22.5, 0, 0, s.r+90);
        GUI.draw.lineWidth = 5;
        GUI.draw.beginPath();
        GUI.draw.strokeStyle = '#8a4b21';
        GUI.draw.moveTo(s.x, s.y);
        const t = this.hostupdate.pt.find(t => t.username === s.team.split(':')[0]);
        if (t) GUI.draw.lineTo(t.x+40, t.y+40);
        GUI.draw.stroke();
  ...
  drawTank(t) {
    ...
    if (t.fire) GUI.drawImage(PixelTanks.images.animations.fire, t.x, t.y, 80, 80, 1, 0, 0, 0, 0, undefined, (this.animate/200)%2, 0, 29, 29);
    GUI.draw.globalAlpha = a;
  ...
  mousemove(e) {
    this.mouse = {x: (e.clientX-(window.innerWidth-window.innerHeight*1.6)/2)*window.innerHeight/1000, y: e.clientY*window.innerHeight/1000};
