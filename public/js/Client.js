class Client {
  ...
  reset() {
    ...
    let grapple = hook ? 5000-hook*2000 : 5000; 
    this.timers = {
  ...
  drawBlock(b) {
    ...
    const size = (b.type === 'airstrike' || b.type === 'smoke' || b.type === 'instastrike') ? 200 : 100, type = ['airstrike', 'fire'].includes(b.type) && Engine.getTeam(this.team) === Engine.getTeam(b.team) ? 'friendly'+b.type : b.type;
    try {
  ...
  frame() {
    ...
    if (Engine.hasPerk(PixelTanks.userData.perk, 6)) {
      ...
      if (player.eradar.length) for (const e of player.eradar) {
        GUI.draw.rotate(e*Math.PI/180);
        GUI.drawImage(PixelTanks.images.menus.arrow, -25, 60, 50, 50, 1);
        GUI.draw.rotate(-e*Math.PI/180);
      }
      if (player.fradar.length) for (const f of player.fradar) {
