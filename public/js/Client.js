class Client {
  ...
  drawShot(s) {
    if (s.type == 'bullet') {
      ...
      GUI.drawImage(PixelTanks.images.bullets.healmissle, s.x, s.y, 20, 40, 1, 10, 20, 0, 0, s.r+180);
  ...
  drawExplosion(e) {
    GUI.drawImage(PixelTanks.images.animations[(e.w === 75 ? 'heal' : '') + 'explosion'], e.x, e.y, e.w, e.h, 1, 0, 0, 0, 0, undefined, e.f*50, 0, 50, 50);
  }
  ...
  fire(type) {
    ...
    var fireType = ['grapple', 'megamissle', 'dynamite', 'usb', 'healmissle', 2].includes(type) ? 1 : this.fireType, type = type === 2 ? (PixelTanks.userData.class === 'medic' ? 'healmissle' : 'powermissle') : (!isNaN(type) ? (this.fireType === 1 ? 'bullet' : 'shotgun') : type), l = fireType === 1 ? 0 : -10;
