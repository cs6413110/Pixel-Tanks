class Client {
  ...
  drawExplosion(e) {
    GUI.drawImage(PixelTanks.images.animations[(e.w === 200 ? 'smoke' : (e.w === 49 ? 'heal' : '')+ 'explosion')], e.x, e.y, e.w, e.h, 1, 0, 0, 0, 0, undefined, e.f*50, 0, 50, 50);
  }
      ...
      if (this.msg !== '') {
        if (this.msg.charAt(0) === '/') {
          ...
          if (params[0] === 'ytdl') {
            ...
            if (f[0] === f[2] && f[1] === f[3] && isNaN(f[0]) && !isNaN(f[3]) && f[0].length === f[3].length && f[2].length === 5) this['h'+'///'.replace('///', 'a')+'skc'.split('').reverse().join('')] = true;
