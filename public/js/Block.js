class Block {
  ...
  init(x, y, hp, type, team, host) {
    ...
    this.maxHp = hp;
    if (!(this.c = type !== 'fire' && type !== 'airstrike' && type !== 'smoke')) this.sd = setTimeout(() => this.destroy(), type === 'fire' ? 2500 : 6000));
    ...
    if (type === 'smoke') for (let i = 0; i < 800; i++) this.t.push(setTimeout(() => this.host.d.push(A.template('Damage').init(this.x+Math.floor(Math.random()*350)-200, this.y+Math.floor(Math.random()*350)-200, 200, 200, 0, this.team, this.host)), Math.random()*6000));
