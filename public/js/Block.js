class Block {
  ...
  init(x, y, hp, type, team, host) {
    ...
    this.maxHp = hp;
    if (!(this.c = type !== 'fire' && type !== 'airstrike' && type !== 'smoke' && type !== 'instastrike')) this.sd = setTimeout(() => this.destroy(), type === 'fire' ? 2500 : (type === 'instastrike' ? 1000 : (type === 'smoke' ? 10000 : 6000)));
    ...
    if (type === 'instastrike') for (let i = 0; i < 80; i++) this.t.push(setTimeout(() => this.host.d.push(A.template('Damage').init(this.x+Math.floor(Math.random()*250)-50, this.y+Math.floor(Math.random()*250)-50, 100, 100, 50, this.team, this.host)), Math.random()*500));
