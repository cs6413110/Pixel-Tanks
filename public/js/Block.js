class Block {
  ...
  init(x, y, hp, type, team, host) {
    ...
    if (type === 'smoke') for (let i = 0; i < 800; i++) this.t.push(setTimeout(() => this.host.d.push(A.template('Damage').init(this.x+Math.floor(Math.random()*350)-100, this.y+Math.floor(Math.random()*350)-100, 200, 200, 0, this.team, this.host)), Math.random()*10000));
