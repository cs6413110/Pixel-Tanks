class Shot {
  ...
  collide(e) {
    ...
    if (this.type === 'dynamite' || this.type === 'usb' || this.type === 'grapple') {
      ...
      } else if (this.type === 'usb' && Engine.getTeam(e?.team) !== Engine.getTeam(g.team)) setTimeout(() => this.destroy(), 20000);
      return false;
  ...
  update() {
    ...
    if (Math.floor(this.x/100) !== x1 || Math.floor(this.y/100) !== y2 || Math.floor((this.x+10)/100) !== x2 || Math.floor((this.y+10)/100) !== y2) this.host.loadCells(this, this.x, this.y, 10, 10);
    this.x = x;
    this.y = y;
