class Shot {
  ...
  cellUpdate() {
    ...
    if (Math.floor(this.oldx/100) !== Math.floor(this.x/100) || Math.floor(this.oldy/100) !== Math.floor(this.y/100) || Math.floor((this.oldx+10)/100) !== Math.floor((this.x+10)/100) || Math.floor((this.oldy+10)/100) !== Math.floor((this.y+10)/100)) { 
      ...
      this.host.logs.push({m: `Bullet Cell Updating error occurred. Destroying bullet + logging debug info. Err: `+e+`. Bullet was a ${this.type} bullet. Its coords were ${this.x}, ${this.y} and its old position was ${this.oldx}, ${this.oldy}. Its delta coords were ${this.dx}, ${this.dy}. `, c: '#FF0000'});
      this.destroy();
    }
  }
