    ...
    world1: {
      ...
      listeners: {
        mousedown: function(e) {
          let i = 0;
          for (const c of PixelTanks.levelCoords) if (PixelTanks.userData.levelBeaten >= (++i)+0) if (Engine.collision(Menus.x, Menus.y, 0, 0, c[0], c[1], 200, 100)) {
    ...
    victory2: { // hi aaron, here to fix my code??? that's funny. bc it only took me screaming fix 5000 times and brealking game threee times over for u to do anything lol EZZZZZ. imam get back to lag byeing away from sound effects LAG B&YE
      buttons: [
        [580, 556, 432, 104, () => {
          const data = Menus.menus.victory.data;
          Menus.removeListeners();
          PixelTanks.user.player = new Client(PixelTanks.levelCoords.indexOf(data[0])+1, data[1], data[2]);
        }, true],
        [656, 680, 280, 104, 'main', true],
      ],
      ontrigger: function() {
