...
function Game() {
  ...
  class Tank {
    ...
    drawAI(ai) {
      ...
      if (raw.mx.length === 2) {
        GUI.draw.moveTo(raw.mx[0], raw.my[0]);
        GUI.draw.lineTo(raw.mx[1], raw.my[1]);
      }
      GUI.draw.stroke();
    ...
    collision(x, y) {
      ...
      while (l<len) {
        if ((x > blocks[l].x || x + 80 > blocks[l].x) && (x < blocks[l].x + 100 || x + 80 < blocks[l].x + 100) && (y > blocks[l].y || y + 80 > blocks[l].y) && (y < blocks[l].y + 100 || y + 80 < blocks[l].y + 100)) {
          if ((blocks[l].type === 'fire' || blocks[l].type === 'fortress' && blocks[l].team.split(':')[1] === team)) {} else if (['barrier', 'weak', 'strong', 'gold', 'fortress', 'void'].includes(blocks[l].type)) return false;
        }
        l++;
      }
      return true;
      ...
      if (k === 9) {
        ...
        if (c === 'stealth') {
          ...
          if (this.canInvis)  {
            ...
            this.timers.class = {time: Date.now(), cooldown: 30000-(Date.now()-this.timers.class.time)};
          }
