class Engine {
  ...
  useAbility(t, a) {
    if (a === 'dynamite') {
      ...
      for (let i = -2; i <= 2; i++) {
        const airstrike = A.template('Block').init(Number(h[0])+i*Math.cos(Math.PI*rotation/180)*100, Number(h[1])+i*Math.sin(Math.PI*rotation/180)*100, 'airstrike', Engine.parseTeamExtras(t.team), this);
