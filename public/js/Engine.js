class Engine {
  ...
  useAbility(t, a) {
    if (a === 'dynamite') {
      ...
      for (let i = this.ai.length-1, turrets = 0; i >= 0; i--) if (this.ai[i].role === 0 && Engine.getUsername(this.ai[i].team) === t.username && ++turrets > 20) this.ai[i].destroy();//april fools
    } else if (a === 'bash') {
  ...
  update(data) {
    ...
    if (fire.length) {
      ...
      for (const s of fire) {
        ...
        A.template('Shot').init(t.x+40, t.y+40, s.r, s.type, Engine.parseTeamExtras(t.team), t.rank, this);
      }
    }
  }
