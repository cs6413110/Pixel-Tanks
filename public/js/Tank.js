class Tank {
  ...
  damageCalc(x, y, a, u) {
    ...
    for (let i = Math.max(0, hx-1); i <= Math.min(29, hx+1); i++) for (let l = Math.max(0, hy-1); l <= Math.min(29, hy+1); l++) for (const entity of this.host.cells[i][l]) {
      if (entity instanceof Shot) if (entity.target) if (entity.target.id === this.id && entity.type === 'usb') a = Math.max(0, a+Math.min(Math.abs(a)/10, 11)*(Engine.getTeam(entity.team) === Engine.getTeam(this.team) ? -1 : 1));
    }
